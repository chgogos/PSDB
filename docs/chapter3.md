...
1.  **Εισαγωγή Όρου Αναζήτησης.** Πληκτρολογήστε έναν όρο αναζήτησης (π.χ. "Students", "2018", "Cognitive") στο πεδίο αναζήτησης.
...
## 3.2 Οδηγός Εξαγωγής Δεδομένων PISA
...
*   **Μεγάλες εξαγωγές**: Μπορεί να διαρκέσουν αρκετά λεπτά. Η επιλογή κωδικού κοινής χρήσης είναι πιο αξιόπιστη για μεγάλα σύνολα δεδομένων.
...
1.  **Επιλογή Μεθόδου Ρύθμισης.** Αρχικά, επιλέξτε πώς θέλετε να ρυθμίσετε την εξαγωγή σας:
    ...
    *   `export_option` (string): Είτε `"direct"` (για απευθείας λήψη) είτε `"share"` (για δημιουργία κωδικού κοινής χρήσης).

2.  **Επιλογή Κύκλου.** Επιλέξτε τον κύκλο PISA (π.χ. `PISA_2022`) από την αναπτυσσόμενη λίστα.
    
    ![Επιλογή κύματος PISA για την εξαγωγή](images/chapter3/13.png)
...
4.  **Επιλογή Στηλών.** Σε αυτό το βήμα, εμφανίζεται ένας πίνακας με όλες τις στήλες του επιλεγμένου πίνακα. Μπορείτε να επιλέξετε τις στήλες που θέλετε να συμπεριλάβετε στην εξαγωγή, χρησιμοποιώντας τα πλαίσια ελέγχου. Υπάρχουν επίσης επιλογές για "Επιλογή Όλων των Στηλών" ή "Αποεπιλογή Όλων των Στηλών".
...
Παρέχονται επίσης κουμπιά για να επιστρέψετε σε προηγούμενο βήμα ή να ξεκινήσετε τη διαδικασία από την αρχή.


## 3.3 Εξαγωγή Δεδομένων PISA μέσω Κωδικού Κοινής Χρήσης

Η λειτουργία "Εξαγωγή Δεδομένων PISA μέσω Κωδικού Κοινής Χρήσης" είναι διαθέσιμη **χωρίς να απαιτείται σύνδεση** στην εφαρμογή. Πρόκειται για μια ειδική σελίδα που επιτρέπει σε οποιονδήποτε χρήστη — ακόμα και σε μη εγγεγραμμένους ή αποσυνδεδομένους χρήστες — να κατεβάσει ένα αρχείο εξαγωγής δεδομένων, αρκεί να διαθέτει έναν έγκυρο κωδικό κοινής χρήσης.

### Πώς να Βρείτε τη Σελίδα

Η σελίδα αυτή είναι προσβάσιμη από το μενού πλοήγησης της εφαρμογής ακόμα και όταν **δεν είστε συνδεδεμένοι**. Στο πλαϊνό μενού πλοήγησης, θα βρείτε την επιλογή **"Εξαγωγή μέσω Κωδικού Κοινής Χρήσης"** μαζί με τις επιλογές "Σύνδεση", "Δημιουργία Λογαριασμού", "Ξεχάσατε τον Κωδικό;" και "Επαναφορά Κωδικού".

### Χρήση

1.  **Εισαγωγή Κωδικού.** Στο πεδίο "Εισάγετε Κωδικό Κοινής Χρήσης", πληκτρολογήστε τον κωδικό που σας έχει κοινοποιηθεί (π.χ. `375ef6838f81`).
2.  **Ανάκτηση Αρχείου.** Το σύστημα θα αναζητήσει αυτόματα το αρχείο που αντιστοιχεί στον κωδικό. Εάν βρεθεί, θα εμφανιστεί μήνυμα επιτυχίας και ένα κουμπί **"Λήψη Αρχείου"** με το όνομα του αρχείου.
3.  **Λήψη.** Πατήστε το κουμπί λήψης για να κατεβάσετε το αρχείο CSV στον υπολογιστή σας.

![Εξαγωγή δεδομένων μέσω κωδικού κοινής χρήσης χωρίς σύνδεση](images/chapter3/49.png)

!!! note "Σημείωση"
    Η λειτουργία αυτή είναι ιδανική για περιπτώσεις όπου ένας συνδεδεμένος χρήστης δημιουργεί μια εξαγωγή δεδομένων μέσω του Οδηγού Εξαγωγής (ενότητα 3.2) και στη συνέχεια κοινοποιεί τον κωδικό σε τρίτους, οι οποίοι μπορούν να κατεβάσουν τα δεδομένα χωρίς να χρειάζεται να δημιουργήσουν λογαριασμό.


## 3.4 Τάσεις Μεταβλητών Ανά Κύκλο

Η λειτουργία "Τάσεις Μεταβλητών ανά Κύκλο" επιτρέπει στους χρήστες να εξερευνήσουν την εξέλιξη συγκεκριμένων μεταβλητών σε διάφορους κύκλους των μελετών PISA, εμφανίζοντας τα αποτελέσματα σε ένα διαδραστικό γράφημα.

!!! info "Σημείωση"
    Η ανάλυση των δεδομένων πραγματοποιείται σε επίπεδο δείγματος και όχι πληθυσμού.

Η ανάλυση γίνεται επιλέγοντας με τη σειρά:
...
2.  **Επιλογή Κατηγορίας & Κύκλων.** Επιλέξτε την κατηγορία δεδομένων (π.χ. "Student Questionnaire") και τους κύκλους PISA (π.χ. `PISA_2006`, `PISA_2009`) από τις αντίστοιχες λίστες.
...
4.  **Επιλογή Μεταβλητών.** Εμφανίζεται ένας πίνακας με όλες τις διαθέσιμες στήλες (μεταβλητές) από τους επιλεγμένους πίνακες. Για κάθε μεταβλητή, βλέπετε το όνομά της, την περιγραφή της και τους πίνακες στους οποίους περιέχεται. Επιλέξτε τις μεταβλητές που επιθυμείτε χρησιμοποιώντας τα πλαίσια ελέγχου. Μπορείτε να χρησιμοποιήσετε τα κουμπιά "Επιλογή Όλων" και "Αποεπιλογή Όλων", καθώς και να φιλτράρετε τις μεταβλητές με κείμενο.
    ...
    !!! warning "Προσοχή"
        Μεταβλητές που αναφέρονται στην ίδια πληροφορία ενδέχεται να έχουν διαφορετικό όνομα σε διαφορετικούς κύκλους PISA (π.χ. μια μεταβλητή μπορεί να ονομάζεται `ST04Q01` σε έναν κύκλο και `ST004Q01TA` σε έναν άλλο). Στην παρούσα έκδοση, δεν υποστηρίζεται η αυτόματη αντιστοίχιση μεταβλητών που έχουν διαφορετικό όνομα από κύκλο σε κύκλο.
...
## 3.6 Προσαρμοσμένα Ερωτήματα SQL
...
## 3.8 Αναζήτηση & Ανακάλυψη Στηλών
...
2.  **Επιλέξτε** τα ζεύγη πίνακας:στήλη που θέλετε από τα αποτελέσματα.
...
1.  **Εισαγωγή μοτίβου αναζήτησης.** Στο πεδίο "Εισάγετε ένα μοτίβο regex για φιλτράρισμα στηλών:", πληκτρολογήστε το μοτίβο regex που θέλετε να χρησιμοποιήσετε (π.χ. `SC*`, `*AGE*`, `CM03*`).
...
Η λειτουργία χωρίζεται σε πέντε κύριες καρτέλες: "Όλοι οι Χρήστες", "Εκκρεμείς Εγκρίσεις", "Στατιστικά", "Database Maintenance" και "Διαχείριση Ορατότητας".
...
3.  **Λίστα Χρηστών και Διαχείριση.** Εμφανίζεται μια αναλυτική λίστα των χρηστών. Για κάθε χρήστη, μπορείτε να δείτε λεπτομέρειες και να εκτελέσετε ενέργειες.
    ...
    *   **Αλλαγή Ρόλου**: Αλλαγή του ρόλου του χρήστη μεταξύ "διαχειριστή" και "χρήστη".
    ...
    ![Λεπτομέρειες χρήστη και διαθέμιες ενέργειες](images/chapter3/44.png)
...
2.  **Έγκριση/Απόρριψη.** Για κάθε εκκρεμή χρήστη, μπορείτε να επιλέξετε "Έγκριση" (ενεργοποιώντας τον λογαριασμό του) ή "Απόρριψη" (διαγράφοντας την εγγραφή του).
...
### Καρτέλα "Διαχείριση Ορατότητας"

Αυτή η καρτέλα επιτρέπει στον διαχειριστή να ελέγχει ποιοι κύκλοι PISA και ποιοι πίνακες είναι ορατοί στους χρήστες της εφαρμογής. Οι αλλαγές ορατότητας ισχύουν για όλες τις λειτουργίες που εμφανίζουν δεδομένα (π.χ. Ευρετήριο Δεδομένων, Οδηγός Εξαγωγής κ.λπ.).

1.  **Ορατότητα Κύκλων.** Εμφανίζεται λίστα με όλους τους διαθέσιμους κύκλους PISA. Κάθε κύκλος έχει ένα πλαίσιο ελέγχου — εάν είναι επιλεγμένο, ο κύκλος είναι **ορατός** στους χρήστες. Αποεπιλέγοντας έναν κύκλο, ολόκληρος ο κύκλος (μαζί με τους πίνακές του) αποκρύπτεται.

2.  **Ορατότητα Πινάκων.** Για κάθε κύκλο που παραμένει ορατός, εμφανίζεται αναπτυσσόμενη ενότητα με τους πίνακές του. Μπορείτε να αποκρύψετε μεμονωμένους πίνακες αποεπιλέγοντας το αντίστοιχο πλαίσιο ελέγχου. Πίνακες κύκλων που έχουν αποκρυφθεί δεν εμφανίζονται εδώ.

3.  **Αποθήκευση Αλλαγών.** Αφού ρυθμίσετε την ορατότητα, πατήστε το κουμπί **"Αποθήκευση Αλλαγών"** για να εφαρμοστούν οι αλλαγές.

    ![Διαχείριση ορατότητας κύκλων και πινάκων](images/chapter3/50.png)

!!! warning "Προσοχή"
    Η απόκρυψη ενός κύκλου αποκρύπτει αυτόματα και όλους τους πίνακές του, ανεξάρτητα από τις επιμέρους ρυθμίσεις πινάκων.
